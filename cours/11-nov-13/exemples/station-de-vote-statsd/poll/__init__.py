...
def create_app(initial_config=None):
    app = Flask("poll")
    app.stats_client = statsd.StatsClient('localhost', 8125)
    init_app(app)
    ...
    def after_request(response):
        total_time = int((time.time() - request.start) * 1000)

        if request.url_rule:
            controller = request.url_rule.endpoint
        else:
            controller = "unknown"

        app.stats_client.incr('http_request', tags={
            'controller': controller,
            'status': response.status_code
        })
        app.stats_client.timing('http_response_time', total_time, tags={
            'controller': controller,
            'status': response.status_code
        })
